...
import { getFeeData } from "../data/feeData";
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="tw-text-center tw-mb-6 sm:tw-mb-8">
                ...
                <div className="tw-mt-3 tw-inline-flex tw-items-center tw-gap-2 tw-bg-amber-500 tw-text-white tw-px-5 tw-py-2 tw-rounded-full tw-font-bold tw-text-sm tw-shadow-lg tw-shadow-amber-500/25">
                  💰 Fee Structure {feeData.session}
                </div>
              </div>
                  ...
                  <table className="tw-w-full tw-text-sm">
                    <thead>
                      <tr className="tw-bg-gradient-to-r tw-from-slate-800 tw-to-slate-900">
                        ...
                        {feeData.classes.map((cls, i) => (
                          <th
                            key={i}
                            className="tw-px-4 tw-py-4 tw-text-center tw-text-xs tw-font-bold tw-text-white tw-uppercase tw-tracking-wider tw-border-r tw-border-white/10 last:tw-border-r-0 tw-min-w-[90px]"
                          >
                            {cls}
                          </th>
                        ))}
                    ...
                    <tbody className="tw-divide-y tw-divide-slate-100">
                      {feeData.feeCategories.map((cat, rowIdx) => (
                        <tr
                          key={cat.id}
                          className={`${rowIdx % 2 === 0 ? "tw-bg-white" : "tw-bg-slate-50/60"} hover:tw-bg-amber-50/40 tw-transition-colors`}
                        >
                          ...
                          {feeData.classes.map((cls, colIdx) => (
                            <td
                              key={colIdx}
                              className="tw-px-4 tw-py-4 tw-text-center tw-font-bold tw-text-slate-800 tw-text-sm tw-border-r tw-border-slate-100 last:tw-border-r-0"
                            >
                              {cat.fees[cls] ? (
                                <span>₹{parseFloat(cat.fees[cls]).toLocaleString("en-IN")}</span>
                              ) : (
                      ...
                      {/* Total Row */}
                      <tr className="tw-bg-gradient-to-r tw-from-slate-800 tw-to-slate-900">
                        <td className="tw-px-5 tw-py-4 tw-font-extrabold tw-text-white tw-text-sm tw-uppercase tw-tracking-[2px] tw-sticky tw-left-0 tw-bg-slate-800 tw-z-10">
                          Total
                        </td>
                        {feeData.classes.map((cls, i) => {
                          const total = feeData.feeCategories.reduce(
                            (sum, cat) => sum + (parseFloat(cat.fees[cls]) || 0),
                            0
                          );
                          return (
                            <td key={i} className="tw-px-4 tw-py-4 tw-text-center tw-font-extrabold tw-text-amber-400 tw-text-base">
              ...
              <div className="md:tw-hidden tw-grid tw-grid-cols-1 sm:tw-grid-cols-2 tw-gap-4">
                {feeData.classes.map((cls, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className="tw-bg-white tw-rounded-xl tw-shadow-md tw-border tw-border-slate-200/60 tw-overflow-hidden"
                    >
                      <div className="tw-bg-gradient-to-r tw-from-slate-800 tw-to-slate-900 tw-px-4 tw-py-3 tw-text-center">
                        <h3 className="tw-text-amber-400 tw-text-lg tw-font-extrabold tw-tracking-widest">
                          {cls}
                        </h3>
                      </div>
                      <div className="tw-p-4">
                        {feeData.feeCategories.map((cat) =>
                          cat.fees[cls] ? (
                            <div
                              key={cat.id}
                              className="tw-flex tw-items-center tw-justify-between tw-py-2 tw-border-b tw-border-slate-100 last:tw-border-b-0"
                            >
                        ...
                        <div className="tw-flex tw-items-center tw-justify-between tw-pt-3 tw-mt-2 tw-border-t-2 tw-border-slate-200">
                          <span className="tw-text-xs tw-font-extrabold tw-text-slate-700 tw-uppercase tw-tracking-wider">
                            Total
                          </span>
                          <span className="tw-font-extrabold tw-text-amber-500 tw-text-lg">
            ...
            <div className="tw-text-center tw-py-20">
              <p className="tw-text-slate-400 tw-text-lg">Fee structure not available.</p>
            </div>
          )}
        </div>
      </div>
